...
const EditWoundNotesUndermining = ({ id, formData, doSubmit, callBack }) => {
    const [today] = useState(new Date().toISOString().substr(0, 10));
    const formSchema = yup.object().shape({
        position: yup
            .string().max(50).required('Position is Required').label("Position"),
        distance: yup
            .number().required('Distance is Required').label("Distance").typeError('Distance value must be a Number'),
    });
            ...
            <Form onSubmit={handleSubmit(onSubmit)}>
                <Row>
                    <Col md="6" className="mb-1">
                        <Label className="form-label" for="position">Position </Label>
...
export default EditWoundNotesUndermining;
