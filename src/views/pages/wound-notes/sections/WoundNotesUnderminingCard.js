...
import WoundNotesUnderminingDialog from '../dialogs/WoundNotesUnderminingDialog';

const WoundNotesUnderminingCard = ({ formData }) => {
    ...
    const url = `/wound_notes/${formData.id}/wound_notes_undermining`
    ...
    const doDeleteRecord = (e, id) => {
        ...
        alertUtils.showDeleteDialog(
            () => {
                useApi
                    .delete(`/wound_notes/${formData.id}/wound_notes_undermining/${id}`, {})
                    .then((res) => {
    ...
    return (
        <>
            <Card>
                <CardHeader>
                    <CardTitle tag='h4'>WOUND NOTES UNDERMINING</CardTitle>
                    <Button color='primary' size='sm' onClick={doAddRecord}>
                        Add wound notes undermining
                    </Button>
                </CardHeader>
                <CardBody>
                    <Row>
...
export default WoundNotesUnderminingCard;