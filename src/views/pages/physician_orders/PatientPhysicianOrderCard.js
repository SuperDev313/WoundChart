...
import PatientPhysicianOrderDialog from "./PatientPhysicianOrderDialog";
...
const PatientPhysicianOrderCard = () => {
    const { id } = useParams();
    ...
    const url = `/patients/${id}/physician_orders`
    ...
    const doDeleteRecord = (e, rowId) => {
        alertUtils.showDeleteDialog(
            () => {
                useApi
                    .delete(`/patients/${id}/physician_orders/${rowId}`, {})
                    .then((res) => {
    ...
    const columns = [
        ...
        {
            name: "ORDER TYPE ID",
            selector: (row) => [row.order_type_id],
        ...
        <>
            <Card>
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>Physician Orders</CardTitle>
...
export default PatientPhysicianOrderCard;
