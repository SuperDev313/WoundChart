...
const ProceduresCard = () => {
    ...
    const url = `/patients/${id}/procedures`
    ...
    const doDeleteRecord = (e, rowId) => {
        alertUtils.showDeleteDialog(
          () => {
            useApi
            .delete(`/patients/${id}/procedures/${rowId}`, {})
            .then((res) => {
    ...
    const columns = [
        ...
        {
            name: "Procedure DATE",
            selector: (row) => [formattedDate(row.procedure_date)],
            column_name: "procedure_date",
            sortable: true,
        },
        {
            name: "Procedure performed",
            selector: (row) => [row.procedure_performed],
            column_name: "procedure_performed",
            sortable: true,
        },
        ...
        <>
            <Card>
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>Patient Procedure</CardTitle>
                    <div className='d-flex mt-md-0 mt-1'>
                        <Button className='ms-2' color='primary' onClick={doTogglePopup}>
                            <Plus size={15} />
                            <span className='align-middle ms-50'>Add Procedure</span>
                        </Button>
                    </div>
                </CardHeader>
