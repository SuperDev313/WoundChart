...
const AddAppointment = ({ open, doTogglePopup, recordId, doHandleSaveAndClosePopup }) => {
    ...
    const [patient, setPatient] = useState([{ label: "", value: "" }]);
    ...
    const loadPatients = (inputValue, callback) => {
        useApi
            ...
            .then((response) => {
                const patients = response.data.data.rows.map((patient) => ({
                    value: patient.id,
    ...
    React.useEffect(() => {
        const value = patient.find(r => r.value === parseInt(patient_id));
        if (value) {
            setValue("patient_name", value.label);
        }
    }, [patient_id])
    ...
    const handleSelectedPatient = (value) => {
        ...
        if (value != null) {
            setValue("patient_id", value.value);
            setValue("patient_name", value.label);
        }
        else {
            setValue("patient_id", '');
                    ...
                    <Form onSubmit={handleSubmit(onSubmit)}>
                        <Row>
                            ...
                            <Col className="col-md-12 mb-1">
                                <Label className="form-label" for="patient_id">Patient List</Label>
                                ...
                                <Controller
                                    ...
                                    render={({ field: { onChange, ...field } }) => (
                                        <AsyncSelect
                                            id="patient_id"
                                            invalid={errors.patient_id && true}
                                            {...field}
                                            isClearable={true}
                                ...
                                {errors.patient_id && (
                                    <FormFeedback>{errors.patient_id.message}</FormFeedback>
                                )}
                            </Col>
