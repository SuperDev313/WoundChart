...
const RescheduleDeleteAppointment = ({ open, doTogglePopup, recordId, patientId, doHandleSaveAndClosePopup }) => {
    ...
    React.useEffect(() => {
        ...
        if (recordId > 0) {
            useApi
                .get(`/patients/${patientId}/appointments/${recordId}`, {})
                .then((res) => {
                    console.log(res);
                    const data = res.data.data;
                    setValue("patientId", parseInt(patientId));
    ...
    React.useEffect(() => {
        useApi
            ...
            .then((res) => {
                ...
                // res.data.data.map((r) => {
                //     const option = { label: r.description, value: r.patientId };
                //     items.push(option);
                //     return option;
                // });
    ...
    const onSubmit = (data, isValid) => {
        if (isValid) {
            setLoading(true);
            useApi
                .doRun(recordId > 0 ? "put" : "post", `patients/${patientId}/appointments/${recordId}`, {
...
export default RescheduleDeleteAppointment;
